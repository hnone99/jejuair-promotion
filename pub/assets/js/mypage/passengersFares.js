...
//  특정 택스 유무 검색 
function searchTax(reservationDetail, taxCode){
	let flag = false;
	for(let idx = 0 ; idx < reservationDetail.journeys.length && !flag; idx++) {
		for(let sdx = 0 ; sdx < reservationDetail.journeys[idx].segments.length && !flag; sdx++) {
			for(let fdx = 0 ; fdx < reservationDetail.journeys[idx].segments[sdx].fares.length && !flag; fdx++) {
				for(let rdx = 0 ; rdx < reservationDetail.journeys[idx].segments[sdx].fares[fdx].passengerFares.length && !flag; rdx++) {
					for(let cdx = 0 ; cdx < reservationDetail.journeys[idx].segments[sdx].fares[fdx].passengerFares[rdx].serviceCharges.length && !flag; cdx++) {
						let serviceCharge = reservationDetail.journeys[idx].segments[sdx].fares[fdx].passengerFares[rdx].serviceCharges[cdx];
						if(serviceCharge.type == "TravelFee" && serviceCharge.code == taxCode){
							flag = true;
							break;
						}
					}
...
// OFWS PaxType 검색
function searchPaxType(reservationDetail, paxType) {
	let flag = false;
	
	let paxInfo = (reservationDetail.passengersInfo) ? reservationDetail.passengersInfo :  reservationDetail.passengers; 
	for(let pdx in paxInfo){
		if (paxInfo[pdx].passengerTypeCode == paxType) {
			flag = true;
			break;
		}
	}
