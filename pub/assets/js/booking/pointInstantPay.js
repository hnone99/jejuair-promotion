...
function paymentInstantPoint(){
	if(checkField()){
		...
		let payPoint = Number($("[name=payPoint]").val().replace(/,/g , ''));
		let usablePay = "";
		let deviceType = "";
...
function setPointAmount(basePoint){
	//console.log($("[name=payPoint]").val());
	payPoint = Number($("[name=payPoint]").val().replace(/,/g , ''));
	//console.log(payPoint);
	if((basePoint > payPoint || payPoint > 4000000)){
		$("[name=payPoint]").val(sUtil.setComma(basePoint));
	}else{
		$("[name=payPoint]").val(sUtil.setComma(payPoint));
	}	
}
