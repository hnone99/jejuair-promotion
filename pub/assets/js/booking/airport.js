...
function drawlowestFare(lowestFare , stationType) {
	...
	if(typeof lowestFare == 'undefined'){
		...
		if(lowestFare.currencyCode != 'null' && lowestFare.currencyCode != undefined){
			let printCurrency = lowestFare.currencyCode;
			if(cultureCode.substring(0,2) == 'ko' && printCurrency ==='KRW'){
				printCurrency = '원'
				lastCurrency = printCurrency
			}
			$("span[name=currency]").text(BIZ_COMMONS_SCRIPT.getI18n("0000000307.msg00177")+ printCurrency)
			lastCurrency = printCurrency
		}else{
			$("span[name=currency]").text(BIZ_COMMONS_SCRIPT.getI18n("0000000307.msg00177")+ lastCurrency)
		}
		for(let k=0; k<datePickerContainer.length; k++){
			let dayContainer = datePickerContainer[k];
			for(let i=0; i<dayContainer.childElementCount; i++){		
...
$.fn.sliding = function () {
	...
	//		$("#departureDate").val(departureDate.getFullYear()+"-"+_dm+"-"+_dd)
		...
		//메인 여정 조회 후 뒤로가기 후 최근 검색 노선 클릭시 (모바일웹,앱) 이전 월 노출 안되는 현상 수정 (현재날짜의 달부터 표출되도록 수정)
		let channelForCalendar = $('body').attr("class");
		if(channelForCalendar != 'undefined'){
			if(channelForCalendar == 'isPc'){
				channelForCalendar = "pc";
			}
			datepicker.responsive(channelForCalendar);
		}
