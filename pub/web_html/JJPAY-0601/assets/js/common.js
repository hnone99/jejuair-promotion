...
var tooltip = function () {
  ...
  var tooltipPoY = function tooltipPoY(obj) {
    ...
    $arrow.css({
      left: Math.floor(elemPosLeft - obj.width() / 2 - $arrow.width() / 2) - 1
    });
...
var uiCarousel = function () {
  var makeCaroulsel = function makeCaroulsel(dom) {
    ...
    switch (type) {
      ...
      case 'bookingTicket':
        {
          // data-carousel="bookingTicket" 로 검색
          carousel = new Swiper(dom, {
            slidesPerView: 'auto',
            resistance: true,
            spaceBetween: 12,
  ...
  return {
    init: function init(el) {
      ...
      $el.each(function (i, elem) {
        ...
        setting.dateFormat = $elem.data('date-format') || setting.dateFormat; // ajax
        ...
        if ($elem.attr('data-defaultDate')) {
          var dateDate = JSON.parse($elem.attr('data-defaultDate'));
          $.extend(setting, {
        ...
        if ($elem.data('json-id')) {
          var jsonData = JSON.parse($("#".concat($elem.data('json-id')))[0].textContent);
          var listItem = setDate(jsonData);
          $.extend(setting, setPeriodText(), listItem);
        }
        ...
        $(document).on('click', '[data-reset-date]', function (event) {
          ...
          $(elem).flatpickr(pickerOptions).changeMonth(mon);
        });
      });
    },
  ...
  return {
    init: function init(opt) {
      ...
      if (!$('body').find('.toast').is(':visible')) {
        ...
        $('body').append("<div class=\"toast\"><p class=\"toast__text\">".concat(opt.text, "</p></div>"));
        var settings = {
          delay: opt.delay,
          width: opt.width,
        ...
        options = $.extend(defaultOpt(), settings);
        $('.toast').css((_$$css = {}, _defineProperty(_$$css, options.position, "".concat(100 * -1, "%")), _defineProperty(_$$css, "display", 'block'), _defineProperty(_$$css, "width", window.innerWidth < MOBILE_WIDTH ? options.width.mob : options.width.pc), _defineProperty(_$$css, "opacity", 0), _$$css)); // .attr('tabindex', 0);
  ...
  return {
    init: function init(opt) {
      $(opt).find('.default-message__close').on('click', function (e) {
        ...
        $wrap.stop().slideUp(300, function () {
          $wrap.remove();
        });
      });
    }
...
var CustomEvent = function () {
  ...
  (function () {
    var fullItem = new Swiper('[data-carousel="fullItem"]', {
      spaceBetween: 30,
      effect: 'fade',
      observer: true,
      observeParents: true,
...
var responsiveJs = function () {
  var defaultDevice = 'mobile';
  $('body').on('changeDevice', function (e, dev) {
    if (defaultDevice !== dev) {
      defaultDevice = dev;
      ...
      if (defaultDevice === 'mobile') {
        ...
        datepicker.responsive(defaultDevice); // $('.scrollbar-inner, .select2-results__options').scrollbar('destroy');
      } else {
        // pc start
        setBottom.destroy();
        uiSelect.destroy();
        datepicker.responsive(defaultDevice); // $('.scrollbar-inner, .select2-results__options').scrollbar();
      }
    }
  });
...
$('.gnb').on('scroll', function () {
  ...
  var mainLayoutLogin = $('.main-layout-login')
...
/* 여정카드 */
  // $(document).ready(function () {           
  //   $('.boarding-box').each(function () {
  //     var $temp2 = $(this).height();
  //     $(this).parent().find('.cancellation').css('height', $temp2);
  //   })

  //   if ($("body").hasClass('isMobile')) {
  //     $('.boarding-wrap').each(function () {
  //       var $temp4 = $(this).find('.boarding-box').height();
  //       var $temp5 = $(this).find('.button-wrap2').height();
  //       $(this).css('height', $temp4 + $temp5 + 10);
  //     })
  //     //e.preventDefault();
  //   }      
  // });
